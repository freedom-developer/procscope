...
static int create_thread_proc_files (struct task_struct *thread, struct proc_dir_entry *thread_dir)
{
	// 展示各种ID: 
...
static int create_process_proc_files(struct task_struct *process, struct proc_dir_entry *process_dir)
{
	...
	struct proc_dir_entry *tsk_threads_proc_dir;


	if ((ret = create_thread_proc_files(process, process_dir))) 
		return ret;
	...
	tsk_threads_proc_dir = proc_mkdir("threads", process_dir);
	...
	for_each_thread(process, thread) {
		struct proc_dir_entry *thread_dir;
		char thread_name[16];
		if (thread == process)
			continue;
		snprintf(thread_name, sizeof(thread_name), "%d", task_pid_nr(thread));
		thread_dir = proc_mkdir(thread_name, tsk_threads_proc_dir);
		if (!thread_dir) {
			pr_err("create thread_dir failed\n");
			return -EINVAL;
		}
		if ((ret = create_thread_proc_files(thread, thread_dir))) 
			return ret;
	}
...
int create_procscope_procfs(void)
{
	...
	for_each_process(process) {
		...
		if ((ret = create_process_proc_files(process, tsk_proc_dir))) {
